...
#include <float.h>   // Untuk konstanta DBL_MAX dan DBL_MIN, digunakan untuk inisialisasi rata-rata min/max
...
int main() {
    ...
    // Variabel baru: Inisialisasi rata-rata maksimum dengan nilai double terkecil.
    // Tujuannya agar rata-rata pertama yang dihitung pasti akan lebih besar dari DBL_MIN.
    ...
    // --- Langkah 2: Meminta 'n' bilangan bulat dan mencari min/max/rata-rata terendah/tertinggi ---
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (i > 0) {
            ...
            // Bagian baru: Perbarui rata-rata maksimum
    ...
    // --- Langkah 3: Menampilkan hasil nilai terkecil, terbesar, dan rata-rata terendah/tertinggi ---
    printf("\nNilai terkecil: %d\n", min_val);
    ...
    if (n >= 2) {
        ...
        printf("Rata-rata tertinggi dari dua bilangan berurutan: %.2f\n", max_avg_consecutive); // Output baru
    } else {
